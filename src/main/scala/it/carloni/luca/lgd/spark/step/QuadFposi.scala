...
class QuadFposi(private val ufficio: String) extends AbstractSparkStep {
  ...
  override def run(): Unit = {
    ...
    val oldFposiGen = readCsvFromPathUsingSchema(oldFposiLoadCsvPath, oldFposiLoadPigSchema)
      .select(changeDateFormatUDF(col("datainizioDEF"), Y2_M2_D2Format, Y4M2D2Format).as("DATAINIZIODEF"),
        changeDateFormatUDF(col("dataFINEDEF"), Y2_M2_D2Format, Y4M2D2Format).as("DATAFINEDEF"),
        changeDateFormatUDF(col("dataINIZIOPD"), Y2_M2_D2Format, Y4M2D2Format).as("DATAINIZIOPD"),
        changeDateFormatUDF(col("datainizioinc"), Y2_M2_D2Format, Y4M2D2Format).as("DATAINIZIOINC"),
        changeDateFormatUDF(col("dataSOFFERENZA"), Y2_M2_D2Format, Y4M2D2Format).as("DATASOFFERENZA"),
