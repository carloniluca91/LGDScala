...
class QuadFcoll extends AbstractSparkStep {
  ...
  override def run(): Unit = {
    ...
    val fcoll = readCsvFromPathUsingSchema(fcollCsvPath, fcollLoadPigSchema)
      .withColumn("data_inizio_DEF", changeDateFormatUDF(col("data_inizio_DEF"), D2M3Y4Format, Y4M2D2Format))
      .withColumn("data_collegamento", changeDateFormatUDF(col("data_collegamento"), D2M3Y4Format, Y4M2D2Format))
    ...
    val oldFposi = readCsvFromPathUsingSchema(oldFposiCsvPath, oldFposiLoadPigSchema)
      ...
      .withColumn("datainizioDEF", changeDateFormatUDF(col("datainizioDEF"), Y2_M2_D2Format, Y4M2D2Format))
      .withColumn("dataFINEDEF", changeDateFormatUDF(col("dataFINEDEF"), Y2_M2_D2Format, Y4M2D2Format))
